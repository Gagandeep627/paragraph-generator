...
lorem_form.addEventListener("submit", function (itm) {
    //helps not to reload page:
    itm.preventDefault();
    ...
    //1>>check val : if !, <0 or >0 o/i : rand num
    //2>>else : filter upto val from the text 
    //3>> map from that list make them to html code>>
    //4>>then add to the lorem_text section



    if (val <= 0 || val > 9 || isNaN(val)) {

        console.log("value is :",val);

        const mytext = `<p class = "result">${j}</p>`+`<p class = "result">${text[rand]}</p>`;
        lorem_text.innerHTML = mytext;


    }
    else{
        //taking text upto ->val
        console.log("value is in range >0 and <= 9 :",val);
        let mytext = text.slice(0, val);
        //convert to HTML->CODE
        mytext = mytext.map(function (itemz,i){


            return  `<p class = "result">${i+1}</p>` + `<p class = "result">${itemz}</p>`;
